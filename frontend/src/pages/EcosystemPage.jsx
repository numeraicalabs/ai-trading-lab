    ...
    <tr style={{ borderBottom:`1px solid ${C.border}22` }}>
      <td style={{ padding:'10px 14px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9 }}>
          <div style={{ width:32, height:32, borderRadius:8, fontSize:15,
      ...
      <td style={{ padding:'10px 14px' }}>
        <Badge label={agent.state} color={agent.state==='Live'?C.green:agent.state==='Training'?C.yellow:C.muted}/>
      </td>
      ...
      <td style={{ padding:'10px 14px', fontFamily:'monospace', fontSize:12, color:C.text }}>
        v{agent.model_version||0}
        <div style={{ fontSize:10, color:C.muted }}>{agent.versions_count||0} runs</div>
      </td>
      <td style={{ padding:'10px 14px' }}>
      ...
      <td style={{ padding:'10px 14px' }}>
        <div style={{ display:'flex', gap:5 }}>
          <button onClick={() => onTrain(agent.abbr, false)} disabled={busy[agent.abbr]} style={{
            ...
            opacity: busy[agent.abbr] ? 0.5 : 1 }}>⚡ Train</button>
          <button onClick={() => onTrain(agent.abbr, true)} disabled={busy[agent.abbr]} style={{
            padding:'5px 9px', borderRadius:6, fontSize:10, fontWeight:700, cursor:'pointer',
            background:`${C.yellow}22`, border:`1px solid ${C.yellow}44`, color:C.yellow,
            opacity: busy[agent.abbr] ? 0.5 : 1 }}>↺ Force</button>
...
function JobRow({ job }) {
  const color = { completed:C.green,running:C.yellow,failed:C.red,queued:C.muted,cancelled:C.dim }[job.status]||C.muted
  ...
  return (
    <tr style={{ borderBottom:`1px solid ${C.border}22`, fontSize:11 }}>
      <td style={{ padding:'7px 11px', fontFamily:'monospace', color:C.dim, fontSize:10 }}>
        {job.job_id?.split('-').slice(-1)[0]||'—'}
      </td>
      ...
      <td style={{ padding:'7px 11px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <Badge label={job.status} color={color}/>
          {job.status==='running' && (
            <div style={{ flex:1, minWidth:70 }}>
              <ProgressBar value={job.progress||0} color={C.yellow} height={4}/>
              <div style={{ fontSize:9, color:C.muted, marginTop:1 }}>{job.stage}</div>
            </div>
          )}
        </div>
      </td>
      <td style={{ padding:'7px 11px', color:C.muted, fontFamily:'monospace', fontSize:10 }}>
        {elapsed !== null ? `${elapsed}s` : '—'}
      </td>
      <td style={{ padding:'7px 11px' }}>
        {job.status==='completed' && job.result?.accuracy !== undefined && (
          <span style={{ color:C.green, fontFamily:'monospace', fontSize:11 }}>
            {(job.result.accuracy*100||0).toFixed(1)}%
          </span>
        )}
        {job.status==='failed' && (
          <span style={{ color:C.red, fontSize:10 }}>{job.error?.slice(0,35)}</span>
        )}
      </td>
    </tr>
  )
...
export default function EcosystemPage({ lastMessage }) {
  ...
  useEffect(() => { const id = setInterval(fetchAll, 3500); return () => clearInterval(id) }, [fetchAll])
  ...
  useEffect(() => {
    ...
    if (lastMessage.type === 'training_progress') {
      ...
      setJobs(prev => {
        ...
        if (idx >= 0) { const n = [...prev]; n[idx] = j; return n }
        return [j, ...prev]
      })
    }
    if (lastMessage.type === 'training_queued')  log(`Queued: ${lastMessage.job.agent_abbr}`)
    if (lastMessage.type === 'training_failed')  log(`❌ Failed: ${lastMessage.job.agent_abbr} — ${lastMessage.job.error}`)
  }, [lastMessage])

  const trainAgent = async (abbr, force) => {
    setBusy(b => ({ ...b, [abbr]: true }))
    log(`${force?'Force-retrain':'Train'} ${abbr}…`)
    const cfg  = status?.agents?.find(a => a.abbr === abbr)
    const h    = cfg?.horizon || globalH
    const res  = await api.trainAgent(abbr, { symbol:'SPY', horizon:h, force_retrain:force })
    if (res?.job_id) { log(`Job: ${res.job_id}`); setJobs(j => [res, ...j]) }
    else log(`Error queuing ${abbr}`)
    setBusy(b => ({ ...b, [abbr]: false }))
  }

  const trainAll = async (force) => {
    log(`Queue all (${globalH}, force=${force})…`)
    const res = await api.trainAll({ horizon:globalH, force_retrain:force, agents:[] })
    if (res?.queued) { log(`✅ Queued ${res.queued} agents`); await fetchAll() }
    else log('Error queuing all')
  }
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
        <div>
          ...
          <p style={{ color:C.muted, fontSize:12, margin:'4px 0 0' }}>Train · Monitor · Version · Auto-improve</p>
        </div>
        <div style={{ display:'flex', gap:7, flexWrap:'wrap', alignItems:'center' }}>
          {HORIZONS.map(h => (
            <button key={h.id} onClick={() => setGlobalH(h.id)} style={{
              padding:'6px 11px', borderRadius:7, fontSize:11, cursor:'pointer',
              background:  globalH===h.id ? `${h.color}22` : C.surface,
              border:`1px solid ${globalH===h.id?`${h.color}66`:C.border}`,
              color: globalH===h.id ? h.color : C.muted,
            }}>{h.label}</button>
          ))}
          <button onClick={() => trainAll(false)} style={{ padding:'8px 16px', borderRadius:8, cursor:'pointer',
            background:`${C.accent}22`, border:`1px solid ${C.accent}66`, color:C.accent, fontSize:12, fontWeight:700 }}>
            ⚡ Train All</button>
          <button onClick={() => trainAll(true)} style={{ padding:'8px 16px', borderRadius:8, cursor:'pointer',
            background:`${C.yellow}22`, border:`1px solid ${C.yellow}66`, color:C.yellow, fontSize:12, fontWeight:700 }}>
            ↺ Retrain All</button>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10, marginBottom:20 }}>
        {[
          ...
          ['Jobs Total',    jobs.length,                   C.accent],
      ...
      {agents.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:20 }}>
          <Card>
            <SectionTitle title="Model Accuracy" sub="Latest trained model %"/>
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={accData} barSize={22}>
                ...
                <Tooltip {...TT}/>
                <Bar dataKey="acc" name="Accuracy %" fill={C.accent} radius={[4,4,0,0]}
                     label={{position:'top',fontSize:9,fill:C.muted,formatter:v=>`${v.toFixed(0)}%`}}/>
              </BarChart>
            </ResponsiveContainer>
          </Card>
          <Card>
            <SectionTitle title="Improvement vs Prev Version"/>
            {impData.length > 0 ? (
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={impData} barSize={22}>
                  ...
                  <Bar dataKey="delta" fill={C.green} radius={[4,4,0,0]}
              ...
              <div style={{ height:140, display:'flex', alignItems:'center', justifyContent:'center', color:C.muted, fontSize:12 }}>
                Train agents to see improvement deltas
              </div>
            )}
          </Card>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:20 }}>
          ...
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tbody>
                ...
                {jobs.length === 0 && (
                  <tr><td colSpan={6} style={{ padding:'20px 11px', textAlign:'center',
                    color:C.muted, fontSize:11 }}>No jobs yet — click Train All</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <Card>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
            ...
            <button onClick={() => setLogs([])} style={{ fontSize:10, background:'none',
              border:`1px solid ${C.border}`, borderRadius:5, padding:'3px 7px',
              color:C.muted, cursor:'pointer' }}>Clear</button>
          </div>
          <div style={{ height:250, overflowY:'auto', fontFamily:'monospace', fontSize:10,
                        display:'flex', flexDirection:'column', gap:2 }}>
            {logs.length === 0
              ? <div style={{ color:C.muted, padding:'20px 0', textAlign:'center' }}>Waiting for activity…</div>
              : logs.map((m,i) => <div key={i} style={{ color:C.muted, lineHeight:1.5,
                  borderBottom:`1px solid ${C.border}11`, paddingBottom:2 }}>{m}</div>)
            }
          </div>
        </Card>
      </div>
      ...
      {/* AI commentary popup */}
      {commentary.abbr && (
        <div style={{ position:'fixed', bottom:90, right:24, width:360, background:'#0d1525',
                      border:`1px solid ${C.border}`, borderRadius:12, padding:18,
                      zIndex:200, boxShadow:'0 8px 32px #000a' }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}>
            <span style={{ fontSize:12, fontWeight:700, color:C.text }}>🤖 Commentary — {commentary.abbr}</span>
            <button onClick={() => setCommentary({ abbr:'', text:'' })}
